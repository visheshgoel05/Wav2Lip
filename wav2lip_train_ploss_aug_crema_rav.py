...
emotion_dict_crema = {'NEU':0, 'HAP':2,  'SAD':3,'ANG':4,'FEA':5, 'DIS':6}
intensity_dict__crema = {'XX':0, 'LO':1, 'MD':2, 'HI':3}
emotion_dict = {'01':0, '02':1, '03':2, '04':3, '05':4, '06':5, '07':6, '08':7}
intensity_dict = {'01':0, '02':1 }
emonet_T = 5
...
class Dataset(object):
    def __init__(self, split, val=False):
        ...
        for filename in self.all_videos:
            ...
            if "-" in filename :
                labels = splitext(filename)[0].split('-')
                emotion = emotion_dict[labels[2]]
                
                emotion_intensity = intensity_dict[labels[3]]
            else:
                labels = splitext(filename)[0].split('_')
                emotion = emotion_dict_crema[labels[2]]
                
                emotion_intensity = intensity_dict__crema[labels[3]]
            if val:
    ...
    def __getitem__(self, idx):
        while 1:
            ...
            emotion = to_categorical(emotion, num_classes=8)
...
if __name__ == "__main__":
    ...
    writer = SummaryWriter('runs_emo_rav_crema/exp1')
